...
// AI design is temporarily disabled to avoid package dependency issues during static build
...
export default function DesignCardPage() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    // Stubbed design function to avoid Genkit dependency during build.
    try {
      const output = {
        cardImageDataUri: 'https://via.placeholder.com/540x340.png?text=NEP+Card+Preview',
      };
      setResult(output);
      toast({
        title: "Card Designed (Mock)",
        description: "A placeholder membership card preview has been generated.",
      });
    } catch (error) {
      console.error(error);
      toast({
