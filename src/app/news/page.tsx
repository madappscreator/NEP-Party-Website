            ...
            <Card key={article.id} className="flex flex-col">
              <CardHeader>
                {image && (
                 ...
                 <CardTitle className="mt-4">{articleTitle}</CardTitle>
              </CardHeader>
              <CardContent className="flex-grow">
                <p className="text-sm text-muted-foreground">{article.date} - {article.source}</p>
                <p className="text-sm mt-2 line-clamp-3">{articleContent}</p>
              </CardContent>
              <CardFooter>
                <Button variant="link" className="p-0" onClick={() => setSelectedArticle({...article, title: articleTitle, content: articleContent})}>
                  {t('news_read_more')}
                </Button>
              </CardFooter>
            </Card>
